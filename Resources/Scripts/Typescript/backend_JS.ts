...
function carusel() {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            this.response.forEach((item: index, pos: number) => {
...
function buildList(number: number) {
    let text: string;
    text = "";
...
function readListItem(item: index, pos: number) {
...
function buildCarousel(carouselItems: projects, pos: number) {
    let str: string;
    if (pos === 0) {
...
function addCaruselToHTML(html: string) {
...
function readSmallProjectIndex(item: index) {
    let request: XMLHttpRequest = new XMLHttpRequest();
    let changed: boolean = true;
    request.open("GET", "Resources/Texts/" + item.title + ".json", true);
    request.responseType = 'json';
...
function buildSmallProjects(smallProject: projects) {
    let str: string;
...
function addSmallProjectToHTML(html: string) {
